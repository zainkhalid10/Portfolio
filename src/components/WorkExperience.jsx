        ...
        <ol className="relative border-l-2 sm:border-l-4 border-purple-300">
          {workExperience.map((work, idx) => (
            <li key={idx} className="mb-8 sm:mb-10 ml-4 sm:ml-6">
              <span className={`absolute -left-2.5 sm:-left-3 flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 text-xs sm:text-sm ${isDark ? 'bg-purple-600 hover:bg-purple-500 text-white ring-purple-900' : 'bg-purple-600 hover:bg-purple-500 text-white ring-white'} rounded-full ring-2 sm:ring-4 font-bold transition-all duration-300`}>{workExperience.length - idx}</span>
              <div className={`${isDark ? 'bg-purple-900/50 hover:bg-purple-900/70 text-purple-100' : 'bg-gradient-to-br from-purple-50 to-indigo-50 hover:from-purple-100 hover:to-indigo-100 text-gray-800'} rounded-xl shadow hover:shadow-xl transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 p-4 sm:p-6 cursor-pointer`}>
